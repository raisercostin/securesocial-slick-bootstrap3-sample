 ...
 * }}}
 */
object AppAssets extends AssetsBuilder2

class AssetsBuilder2 extends Controller {
  ...
  private[controllers] def resourceNameAt(path: String, file: String): Option[String] = {
    ...
    val a = if (new File(resourceName).isDirectory || !new File(resourceName).getCanonicalPath.startsWith(new File(path).getCanonicalPath)) {
      None
    } else {
      Some(resourceName)
    }
    println(s"2 path=$path file=$file => a=$a sau resource=${new File(resourceName).getAbsolutePath()}")
    a
  }
